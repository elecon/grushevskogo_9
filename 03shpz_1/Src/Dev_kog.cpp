...
//		else enh_info&=~KOG_OS_ERROR;
	...
	void KOG::show_lcd()
	{
		...
		if(enh_info&KOG_DIST) LCD_abc((char*)mode_str[2],12);
		else if(enh_info&KOG_OPER) LCD_abc((char*)mode_str[1],12);
		else LCD_abc((char*)mode_str[0],12);

		if(enh_info&(KOG_PRESS_ERROR|KOG_MP_ERROR|KOG_KTR_ERROR|KOG_OS_ERROR))
			LCD_abc((char*)kog_error_str[0],20);//16
		if(enh_info&KOG_PRESS_ERROR) LCD_abc((char*)kog_error_str[1],23);//19
		if(enh_info&KOG_MP_ERROR) LCD_abc((char*)kog_error_str[2],27);//23
		if(enh_info&KOG_KTR_ERROR) LCD_abc((char*)kog_error_str[3],30);//26
		if(enh_info&KOG_OS_ERROR) LCD_abc((char*)kog_error_str[4],34);//30
