...
//    #define TAMPLE
	...
	enum {ZOVT_C_CLOSE,ZOVT_C_OPEN};
	enum {  ZOVT_ST_CLOSE,   //0x01
            ZOVT_ST_OPEN,    //0x02
            ZOVT_ST_OPER,    //0x04
            ZOVT_ST_DIST,    //0x08
            ZOVT_ST_AVTO,    //0x10
...
//-----------------------------------------------------------------------------------------------
            ...
            void drv()
            {
                if (stan&(KOGT_OPER|KOGT_DIST)) {
                    if (stad&(1<<ZOVT_ST_OPEN)) {
                        stad&=~((1<<ZOVT_ST_OPEN));
                        stan |= KOGT_OP;
                        stan &=~ KOGT_CLOSE;
                        //OUT_FIELD[OUT_BIT]=0xff;
                    }
                    if (stad&(1<<ZOVT_ST_CLOSE)) {
                        stad&=~(1<<ZOVT_ST_CLOSE);
                        stan &=~ KOGT_OP;
                        stan |= KOGT_CLOSE;
                        //OUT_FIELD[OUT_BIT]=0x00;
                    }

                    if (stad&(1<<ZOVT_ST_AVTO)) {
                        stad&=~(1<<ZOVT_ST_AVTO);
                    ...
                    if (stad&(1<<ZOVT_ST_OPER)) {
                        stad&=~(1<<ZOVT_ST_OPER);
                        stan|=KOGT_OPER;
                    }
                    else {
                        if (stad&(1<<ZOVT_ST_DIST)) {
                            stad&=~(1<<ZOVT_ST_DIST);
                            stan|=KOGT_DIST;
                        }
                    }
...
//                else {
                ...
                if (stan&KOGT_OP) LED_FIELD[LED_NOMER]=red_lamp;
            ...
            void show_lcd()
            {
                ...
                if (stad&(1<<ZOVT_C_OPEN)) LCD_abc((char*)stad_str[0],4);
                if (stad&(1<<ZOVT_C_CLOSE)) LCD_abc((char*)stad_str[1],4);
...
//                }
                ...
                if (stan&KOGT_OPER) {
                    ...
                    if (KEY_1) stad|=(1<<ZOVT_ST_ERR);
                    if (KEY_4) {
                        stad|=(1<<ZOVT_ST_AVTO);
                    }
                }
                else {
                    if (!(stan&(KOGT_OPER|KOGT_DIST))) if (KEY_7) stad|=(1<<ZOVT_ST_OPER);
                    if (stan&(KOGT_DIST|KOGT_OPER)) if (KEY_4) stad|=(1<<ZOVT_ST_AVTO);
                }
            }
...
//-----------------------------------------------------------------------------------------------
    ...
    template <  uint16_t LED_NOMER,
                ...
                uint8_t NET_BITE,
                uint8_t NET_FLAG>
	class ZOVT_NET : public TDevObject
	{
	  public:
            //uint8_t mmmm;
            ...
            void drv ()
            {
                ...
                if (stan&KOGT_OP) stad&=~(1<<ZOVT_ST_OPEN);
                if (stan&KOGT_CLOSE) stad&=~(1<<ZOVT_ST_CLOSE);
...
//                }
                if (!(stan&(KOGT_OPER|KOGT_DIST))) stad&=~(1<<ZOVT_ST_AVTO);
                if (stan&KOGT_OPER) stad&=~(1<<ZOVT_ST_OPER);
