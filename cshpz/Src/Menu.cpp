...
//	LCD_uind(data_from_slave02.head.adr,11,2|LEAD_ZERO);
//	LCD_uind(NET_WORK_SHPZ18,14,2|LEAD_ZERO);
...
////////////////////////////////////////////////////////////////!!!доделать!!!///////////
	if ((fire_floor==0xff)&&(!(fire_fl&FIRE_FL_M_ALL))){
        ...
        LCD_abc((char*)fire_str,20);
        if (!(fire_floor==0xff)) LCD_uind(fire_floor,28,2);
//		if (scafe_fl&FIRE_ZONE) 	LCD_abc((char*)fire_str1,20);
//		if (scafe_fl&FIRE_ZONE_PK) 	LCD_abc((char*)fire_str1,20);
//		LCD_abc((char*)fire_str,20); LCD_uind(fire_floor,28,2);
	}
////////////////////////////////////////////////////////////////
//    LCD_uind(fire_floor1,11,3|LEAD_ZERO);
//    LCD_uind(fire_floor2,31,3|LEAD_ZERO);
//	pause_pisk();
...
const prog_char net_test_str[][14]=
{
	"0_ЩПЗ_02",
	"1_ЩПЗ_3",
	"2_ЩПЗ_6",
...
uint8_t scaf_counter;
void show_scaf()
{
	scaf_counter=getpm(scaf_counter,NUM_OF_SCAF+1);
	pMenu_arr[cur_menu]->next_menu = scaf_counter+EMenuSHPZ_02;

	LCD_putc('>',0);

	LCD_abc((char*)net_test_str[scaf_counter],1);

	if (scaf_counter==(NUM_OF_SCAF)) {
		LCD_abc((char*)net_test_str[0],21);
	}
	else {
		LCD_abc((char*)net_test_str[scaf_counter+1],21);
	}
}
...
void show_net_test()
{
	net_counter=getpm(net_counter,NUM_OF_SCAF);
	pMenu_arr[cur_menu]->next_menu = net_counter+EMenuSHPZ_02;

	LCD_abc((char*)net_test_str[net_counter],0);

	LCD_uind(NetPackInfo[net_counter],12,2|LEAD_ZERO);

	if (net_counter==(NUM_OF_SCAF-1)) {
		LCD_abc((char*)net_test_str[0],20);
		LCD_uind(NetPackInfo[0],32,2|LEAD_ZERO);
	}
	else {
		LCD_abc((char*)net_test_str[net_counter+1],20);
		LCD_uind(NetPackInfo[net_counter+1],32,2|LEAD_ZERO);
	}
}
...
//				NAME				SHOW_FUNC			INDEX		BACK		NEXT
	...
	SET_MENU(DEV_MENU_screen,		show_scaf,			EMenuDevs, 		0,	EMenuSHPZ_02)	//2		2
