...
//        for (i=0;i<80;i++) {
//                if (mnemo_counter) LED_FIELD[80*(mnemo_counter)+i]=0xff;
//                else LED_FIELD[80*(NUM_OF_MNEMO)+i]=0xff;
//        }
//	}
    if (scafe_fl&MNEMO_WRK) {
...
void Message_drv()
{
	...
	//------------------------------------
	if(fire_fl&FIRE_FL_M_PK1) message_arch_byte[1]=1;
	else message_arch_byte[1]=0;
	if(fire_fl&FIRE_FL_M_PK2) message_arch_byte[2]=1;
	else message_arch_byte[2]=0;
	if(fire_fl&FIRE_FL_M_PK3) message_arch_byte[3]=1;
	else message_arch_byte[3]=0;
	if(fire_fl&FIRE_FL_M_SPS1) message_arch_byte[4]=1;
	else message_arch_byte[4]=0;
	if(fire_fl&FIRE_FL_M_SPS2) message_arch_byte[5]=1;
	else message_arch_byte[5]=0;
	if(fire_fl&FIRE_FL_M_SPS3) message_arch_byte[6]=1;
	else message_arch_byte[6]=0;
	if(fire_fl&FIRE_FL_M_ALL) message_arch_byte[7]=1;
	else message_arch_byte[7]=0;
	if(fire_fl&FIRE_FL_M_STOP_PDZ) message_arch_byte[8]=1;
	else message_arch_byte[8]=0;
	message_arch_byte[9]=fire_floor;
	//------------------------------------
...
extern const prog_char message_str[NUM_OF_MESSAGE][20] PROGMEM;
const prog_char message_str[NUM_OF_MESSAGE][20]=
{
	"Старт системи",
	"Пож. 1",
	"Пож. 2",
	...
	"Стоп ПДЗ",
	"Пож.пов.",
/*	"Сеть ЩПЗ2П1",///////////////////включить после наладки сети!!!!!!!!!!!!!!1
	"Сеть ЩПЗ2П2",
	"Сеть ЩПЗ2П3",
	"Сеть ЩПЗ2П4",
	"Сеть ЩПЗ2П5",
